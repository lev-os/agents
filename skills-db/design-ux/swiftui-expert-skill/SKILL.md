...
- **@Observable over ObservableObject** — New code prefers @Observable; because modern, nesting works, @State binds.
- **State wrapper selection** — @State (private, owned), @Binding (child modifies), @Bindable (injected @Observable); because wrong wrapper causes bugs.
- **Modifiers over conditionals** — For state changes; because maintains view identity; better diffing.
- **Modern API replacements** — foregroundStyle, clipShape, Tab, Button, NavigationStack; because deprecated APIs have pitfalls.
- **Extract complex views** — Small subviews; because readability and performance.
- **Stable ForEach identity** — Never .indices for dynamic content; because identity churn causes rebuild storms.
- **Liquid Glass only when requested** — Don't push iOS 26+; because fallback maintenance; user choice.

## Technique Notes

References: state-management, view-structure, performance-patterns, list-patterns, modern-apis, liquid-glass. Philosophy: facts and best practices, not architectural enforcement. Use Self._printChanges() for update debugging.

---

## Prompt Architect Overlay

**Role Definition:** SwiftUI expert for state management, view composition, performance, modern APIs, Swift concurrency, iOS 26+ Liquid Glass. Review, improve, or implement SwiftUI features.

**Input Contract:** Accepts code review, refactor request, new feature implementation, or "improve this SwiftUI." Code or feature description. Project context (iOS version, patterns).

**Output Contract:** Checklist-based review or implementation guidance. Property wrapper recommendations. Modern API replacements. Performance notes. Liquid Glass patterns if requested. Reference pointers.

**Edge Cases & Fallbacks:** If legacy ObservableObject→note migration path; avoid forcing. If performance issue→reference performance-patterns.md. If Liquid Glass→gate with #available(iOS 26); provide fallback. If list issues→check ForEach identity.
