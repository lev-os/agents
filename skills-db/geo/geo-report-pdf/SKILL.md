...
- The PDF generation script is located at: `~/.agents/skills-db/geo/geo/scripts/generate_pdf_report.py`
...
```bash
python3 ~/.agents/skills-db/geo/geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json GEO-REPORT-[brand].pdf
```
...
6. **Run the PDF generator**:
   ```bash
   python3 ~/.agents/skills-db/geo/geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json "GEO-REPORT-[brand_name].pdf"
   ```
