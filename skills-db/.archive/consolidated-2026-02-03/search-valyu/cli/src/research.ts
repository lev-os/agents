...
export async function recursiveResearch(
  ...
  const allSources = new Map<string, { title: string; url: string; count: number }>();
  ...
  for (let turn = 1; turn <= maxTurns; turn++) {
    ...
    try {
      ...
      const searchResult = await client.search({
        query: currentQuery,
        max_num_results: strategy === 'breadth' ? 20 : strategy === 'depth' ? 10 : 15,
        search_type: 'all',
        response_length: strategy === 'depth' ? 'large' : 'medium',
      });

      const answerResult = await client.answer({
        query: currentQuery,
        system_instructions: `You are conducting turn ${turn} of ${maxTurns} in a recursive research process.
...
NEXT_QUERY: [refined query for next turn, or "COMPLETE" if done]`,
      ...
      totalCost += answerResult.cost.total_dollars;
      ...
      const response = answerResult.contents as string;
      ...
      const turnResult: TurnResult = {
        ...
        cost: searchResult.total_deduction_dollars + answerResult.cost.total_dollars,
      ...
      writeFileSync(
        join(outputDir, `turn-${turn}.json`),
        JSON.stringify({ turnResult, searchResult, answerResult }, null, 2)
      );

      writeFileSync(
        join(outputDir, `turn-${turn}.md`),
        `# Turn ${turn}: ${currentQuery}\n\n` +
        `**Confidence:** ${(currentConfidence * 100).toFixed(1)}%\n` +
        `**Sources:** ${searchResult.results.length}\n` +
        `**Cost:** $${turnResult.cost.toFixed(4)}\n\n` +
        `## Findings\n\n${findings}\n\n` +
        `## Sources\n\n${searchResult.results.map((s: any, i: number) => `${i + 1}. [${s.title}](${s.url})`).join('\n')}\n`
      );

      spinner.succeed(
        `Turn ${turn} complete: ${(currentConfidence * 100).toFixed(0)}% confidence, ${searchResult.results.length} sources, $${turnResult.cost.toFixed(4)}`
      );
  ...
  const synthesisResult = await client.answer({
    query: `Based on ${turns.length} research turns on "${query}", provide a comprehensive final synthesis.
...
4. Areas requiring further research (if any)`,
    system_instructions: 'You are synthesizing multi-turn recursive research. Be comprehensive, cite sources, and indicate certainty levels.',
  });

  totalCost += synthesisResult.cost.total_dollars;
  ...
  const result: ResearchResult = {
    ...
    totalCost,
    synthesis: synthesisResult.contents as string,
    keySources,
    turns,
  };
...
| **Synthesis** | Final synthesis | $${synthesisResult.cost.total_dollars.toFixed(4)} | - |
