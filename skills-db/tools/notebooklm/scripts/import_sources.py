...
def fetch_guide(source_id: str) -> tuple[str, list[str], list[str]]:
    ...
    try:
        ...
        return data.get("summary", ""), data.get("topics", []), data.get(
            "keywords", []
        )
    except Exception:
        return "", [], []
...
def main() -> None:
    ...
    parser.add_argument("--sources", required=True, help="Path to source list JSON")
    ...
    parser.add_argument(
        "--skip-guides",
        action="store_true",
        help="Skip fetching AI source guides",
    )
