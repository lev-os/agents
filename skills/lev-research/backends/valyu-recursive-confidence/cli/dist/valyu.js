...
const getClient = () => {
    ...
    if (!apiKey) {
        ...
        console.error(chalk.yellow('Set it in ~/.env.local or export VALYU_API_KEY=your-key'));
        process.exit(1);
    }
...
program
    ...
    try {
        const results = await client.search(query, {
            maxNumResults: parseInt(options.num),
...
program
    ...
    try {
        ...
        if ('success' in result && result.success) {
            ...
            console.log(chalk.bold.cyan('\n📚 Sources:'));
            result.search_results.slice(0, 5).forEach((source, i) => {
                console.log(chalk.gray(`[${i + 1}] ${source.title}`));
                console.log(chalk.gray(`    ${source.url}`));
            });
            console.log(chalk.gray(`\nCost: $${result.cost.total_deduction_dollars.toFixed(4)}`));
...
program
    ...
    const client = getClient();
    try {
        const results = await client.contents(urls, {
            responseLength: options.length,
            extractEffort: options.effort,
            summary: options.summary || false,
            screenshot: options.screenshot || false,
        });
        spinner.succeed('Content extracted');
        console.log(chalk.bold(`\n📄 Processed ${results.urls_processed ?? 0}/${results.urls_requested ?? urls.length} URLs`));
        console.log(chalk.gray(`Cost: $${(results.total_cost_dollars ?? 0).toFixed(4)}`));
        (results.results ?? []).forEach((result, i) => {
            console.log(chalk.bold.cyan(`\n[${i + 1}] ${result.title}`));
            console.log(chalk.gray(result.url));
