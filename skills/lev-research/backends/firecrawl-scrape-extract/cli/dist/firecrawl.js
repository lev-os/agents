...
const saveOutput = (data, output, format = 'json') => {
    if (output) {
...
const pollJob = async (client, endpoint, jobId, spinner) => {
...
program
    ...
    try {
        ...
        if (options.includeTags) {
            payload.includeTags = options.includeTags.split(',').map((t) => t.trim());
        }
        if (options.excludeTags) {
            payload.excludeTags = options.excludeTags.split(',').map((t) => t.trim());
        }
...
program
    ...
    try {
        ...
        if (options.includePaths) {
            payload.includePaths = options.includePaths.split(',').map((p) => p.trim());
        }
        if (options.excludePaths) {
            payload.excludePaths = options.excludePaths.split(',').map((p) => p.trim());
        }
        ...
        if (crawlResult.data && Array.isArray(crawlResult.data)) {
            crawlResult.data.slice(0, 5).forEach((page, i) => {
                const p = page;
...
program
    ...
    try {
        ...
        if (options.json) {
            ...
            if (options.output) {
                const content = links.map((l) => typeof l === 'string' ? l : l.url).join('\n');
                writeFileSync(options.output, content);
                console.log(chalk.green(`\n✓ Saved to ${options.output}`));
            }
        }
...
program
    ...
    try {
        ...
        if (batchResult.data && Array.isArray(batchResult.data)) {
            batchResult.data.slice(0, 5).forEach((page, i) => {
                const p = page;
                console.log(chalk.cyan(`\n[${i + 1}] ${p.metadata?.title || 'Untitled'}`));
                console.log(chalk.gray(p.metadata?.sourceURL || ''));
            });
    ...
    try {
        ...
        if (options.scrape) {
            payload.scrapeOptions = {
                formats: options.scrapeFormats.split(',').map((f) => f.trim()),
            };
        }
        if (options.tbs) {
