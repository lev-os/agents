...
class AutoresearchRuntimeControllerTest(AutoresearchScriptsTestBase):
    ...
    def test_create_launch_manifest_namespaces_paths_by_run_tag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            created = self.create_launch_manifest(
                tmpdir,
                run_tag="nightly-joint-a",
            )
            launch_path = Path(created["launch_path"])
            self.assertEqual(
                launch_path.resolve(),
                (tmpdir / ".autoresearch" / "runs" / "nightly-joint-a" / "autoresearch-launch.json").resolve(),
            )
            manifest = json.loads(launch_path.read_text(encoding="utf-8"))
            self.assertEqual(manifest["config"]["run_tag"], "nightly-joint-a")
            self.assertIn(".autoresearch/runs/nightly-joint-a", manifest["config"]["artifacts_root"])
    ...
    def test_runtime_launch_allows_multiple_run_tags_in_one_repo(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            fake_codex_path = tmpdir / "fake-codex"
            self.write_sleeping_fake_codex(fake_codex_path)

            launched_a = self.launch_runtime(tmpdir, fake_codex_path=fake_codex_path, run_tag="run-a")
            self.assertEqual(launched_a["status"], "running")
            launched_b = self.launch_runtime(tmpdir, fake_codex_path=fake_codex_path, run_tag="run-b")
            self.assertEqual(launched_b["status"], "running")

            self.assertTrue((tmpdir / ".autoresearch" / "runs" / "run-a" / "autoresearch-runtime.json").exists())
            self.assertTrue((tmpdir / ".autoresearch" / "runs" / "run-b" / "autoresearch-runtime.json").exists())

            stopped_a = self.run_script(
                "autoresearch_runtime_ctl.py",
                "stop",
                "--repo",
                str(tmpdir),
            ...
            self.assertEqual(stopped_a["status"], "stopped")
            stopped_b = self.run_script(
                "autoresearch_runtime_ctl.py",
                "stop",
                "--repo",
                str(tmpdir),
            ...
            self.assertEqual(stopped_b["status"], "stopped")

    def test_list_runs_reports_tagged_runtimes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmpdir = Path(tmp)
            fake_codex_path = tmpdir / "fake-codex"
            self.write_sleeping_fake_codex(fake_codex_path)

            self.launch_runtime(tmpdir, fake_codex_path=fake_codex_path, run_tag="run-a")
            self.launch_runtime(tmpdir, fake_codex_path=fake_codex_path, run_tag="run-b")

            listing = self.run_script(
                "autoresearch_runtime_ctl.py",
                "list-runs",
                "--repo",
                str(tmpdir),
            ...
            self.assertEqual({entry["run_tag"] for entry in listing["runs"]}, {"run-a", "run-b"})
