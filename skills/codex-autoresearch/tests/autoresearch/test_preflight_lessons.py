...
class AutoresearchPreflightLessonsTest(AutoresearchScriptsTestBase):
    ...
    def test_commit_gate_warns_on_dirty_prelaunch_and_staged_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any("unexpected worktree changes before launch" in warning for warning in result["warnings"])
            )
            self.assertTrue(
                any("autoresearch-owned artifacts are staged" in warning for warning in result["warnings"])
            )
            self.assertTrue(
                any(
    ...
    def test_commit_gate_warns_on_dirty_precommit(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            subprocess.run(["git", "init", str(repo)], check=True, capture_output=True, text=True)
            (repo / "notes.txt").write_text("user change\n", encoding="utf-8")
            ...
            self.assertTrue(
                any("unexpected worktree changes before commit" in warning for warning in result["warnings"])
            )
    ...
    def test_commit_gate_warns_on_rename_from_out_of_scope_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            subprocess.run(["git", "init", str(repo)], check=True, capture_output=True, text=True)
            docs = repo / "docs"
    ...
    def test_commit_gate_warns_on_rename_from_artifact_source_into_scope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            subprocess.run(["git", "init", str(repo)], check=True, capture_output=True, text=True)
    ...
    def test_commit_gate_warns_on_out_of_scope_changes_in_companion_repo(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            primary = root / "primary"
            companion = root / "companion"
