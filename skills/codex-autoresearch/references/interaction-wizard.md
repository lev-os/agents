...
- "If this repo will host multiple simultaneous runs, what run tag should I use so the artifacts stay isolated?"
...
- "This repo may stay dirty because of swarm work. Should I use checkpoint mode and always `git add .`, commit, pull, push after each iteration?"
...
- Git mode / rollback (optional) -- ask only if the repo is not already clearly using checkpoint-mode git
