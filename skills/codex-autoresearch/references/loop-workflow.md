...
5. If multiple runs share the repo, check `.autoresearch/checkpoint.lock` and wait if another run currently owns the checkpoint window.
6. Checkpoint the current tree with `git add .`, `git commit`, `git pull`, `git push`.
7. Run the verify command and guard.
8. Record the result through `autoresearch_record_iteration.py` or `autoresearch_select_parallel_batch.py`.
