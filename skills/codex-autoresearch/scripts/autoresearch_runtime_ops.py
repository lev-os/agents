...
def runtime_summary(
    *,
    ...
    launch_context = evaluate_launch_context(
        ...
        run_tag=runtime.get("run_tag") if runtime else None,
    )
    try:
...
def create_launch_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    launch_path = resolve_repo_relative(
        repo,
        args.launch_path,
        default_launch_manifest_path(repo, run_tag),
    )
...
def start_runtime(args: argparse.Namespace, *, runner_path: Path) -> dict[str, Any]:
    ...
    results_path = resolve_repo_relative(
        repo,
        args.results_path,
        default_results_path(repo, run_tag),
    )
    ...
    log_path = resolve_repo_relative(
        repo,
        args.log_path,
        default_runtime_log_path(repo, run_tag),
    )
...
def run_runtime(args: argparse.Namespace) -> int:
    ...
    launch_manifest = read_launch_manifest(launch_path)
    manifest_run_tag = launch_manifest.get("config", {}).get("run_tag") or run_tag
    results_path = resolve_repo_relative(
        repo,
        args.results_path,
        default_results_path(repo, manifest_run_tag),
    )
    runtime_path = resolve_repo_relative(
        repo,
        args.runtime_path,
        default_runtime_state_path(repo, manifest_run_tag),
    )
    log_path = resolve_repo_relative(
        repo,
        args.log_path,
        default_runtime_log_path(repo, manifest_run_tag),
    )
    state_path_arg = args.state_path
...
def stop_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    runtime_path = resolve_repo_relative(
        repo,
        args.runtime_path,
        default_runtime_state_path(repo, getattr(args, "run_tag", None)),
    )
    runtime, runtime_error = load_runtime_with_error(runtime_path)
    if runtime_error is not None:
        return {
...
def list_runs(repo: Path) -> dict[str, Any]:
    repo = resolve_repo_path(str(repo))
    runs_root = repo / ".autoresearch" / "runs"
    entries: list[dict[str, Any]] = []

    singleton_runtime_path = default_runtime_state_path(repo)
    singleton_runtime, singleton_error = load_runtime_with_error(singleton_runtime_path)
    if singleton_runtime is not None or singleton_error is not None:
        entries.append(
            {
                "run_tag": "",
                "runtime_path": str(singleton_runtime_path),
                "status": singleton_runtime.get("status") if singleton_runtime else "needs_human",
                "pid": singleton_runtime.get("pid") if singleton_runtime else None,
                "error": singleton_error,
            }
        )

    if runs_root.exists():
        for runtime_path in sorted(runs_root.glob("*/autoresearch-runtime.json")):
            run_tag = runtime_path.parent.name
            payload, error = load_runtime_with_error(runtime_path)
            entries.append(
                {
                    "run_tag": run_tag,
                    "runtime_path": str(runtime_path),
                    "status": payload.get("status") if payload else "needs_human",
