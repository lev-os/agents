...
def evaluate_commit_gate(
    *,
    ...
    if phase in phase_labels and unexpected_worktree:
        ...
        warnings.append(
            f"unexpected worktree changes {label}: "
            + ", ".join(sorted(unexpected_worktree))
            + " (checkpoint mode allows dirty trees)"
        )
    ...
    if staged_artifacts:
        warnings.append(
            ...
            + ", ".join(sorted(staged_artifacts))
            + " (checkpoint mode allows staged artifacts)"
        )
