...
def build_runtime_prompt(
    *,
    ...
    checkpoint_lock = config.get("checkpoint_lock_path") or str(default_checkpoint_lock_path(primary_repo))
    run_tag = config.get("run_tag") or "untagged"
    lock_helper = Path(__file__).with_name("autoresearch_checkpoint_lock.py")
    lines = [
        "$codex-autoresearch",
    ...
    lines.extend(
        [
            ...
            f"Checkpoint lock path: {checkpoint_lock}",
            f"Checkpoint lock helper: {lock_helper}",
            "",
            "Runtime checklist:",
            *[f"- {item}" for item in RUNTIME_CHECKLIST],
            ...
            f"- Before any `git add .`, `git commit`, `git pull`, or `git push`, run `python3 {lock_helper} acquire --repo {primary_repo} --run-tag {run_tag} --wait-seconds 600`.",
            f"- After the checkpoint sequence completes, run `python3 {lock_helper} release --repo {primary_repo} --run-tag {run_tag}`.",
