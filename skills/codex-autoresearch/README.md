...
### Multi-run artifact layout

When a run includes `run_tag`, the runtime now namespaces its managed artifacts under:

```text
.autoresearch/runs/<run-tag>/
...
Without `run_tag`, the legacy repo-root artifact names are still used.

Use `status --run-tag <tag>` and `stop --run-tag <tag>` to target a specific background run.

### Shared checkpoint lock

Multiple runs may share one repo, but git checkpointing is still serialized through:

```text
.autoresearch/checkpoint.lock
```

Before any `git add .`, `git commit`, `git pull`, or `git push`, a run should check this lock. If another run holds it, wait and retry instead of checkpointing concurrently.
...
|---------|-------------------|
| Dirty worktree | Runtime preflight warns but allows launch in checkpoint-mode repos where dirty trees are expected |
...
| Failed change | Checkpoint the failure in git, mark it `discard` / `crash` in the results log, then recover forward with the next commit |
