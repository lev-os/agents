...
For unattended runs, the wizard may also ask one safety question about git mode. In checkpoint-mode repos, dirty worktrees are expected and the default iteration sequence is `git add .`, commit, pull, push. After you say "go," it stays silent.

If you run multiple autoresearch jobs in the same repo, give each run a unique `run_tag`. That places its managed artifacts under `.autoresearch/runs/<run-tag>/` instead of repo-root singleton files. Shared git checkpointing is still coordinated through `.autoresearch/checkpoint.lock`.
...
In checkpoint-mode repos, failed experiments are usually kept as checkpoint commits and marked `discard` / `crash` in the results log. Prefer forward recovery over reset/revert.
