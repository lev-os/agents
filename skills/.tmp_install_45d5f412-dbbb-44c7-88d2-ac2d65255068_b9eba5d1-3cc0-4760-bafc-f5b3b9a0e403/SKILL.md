...
# Health + refresh (if health fails or stale index)
cass status --json && cass index --json
...
Use `--robot-format sessions` for pipeline workflows:

```bash
# Find sessions, then expand each match
cass search "auth error" --robot-format sessions | head -5 | while read session; do
  cass expand "$session" --line 1 --context 2 --json
done

# Filter sessions to recent work, then view
cass search "refactor" --robot-format sessions --limit 20 | while read s; do
  cass view "$s" -n 1 -C 10
done
```
